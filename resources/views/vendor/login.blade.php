@extends('layouts.base', ['title' => 'Login'])

@section('css')

@endsection

@section('content')
    <div class="relative min-h-screen w-full flex justify-center items-center py-16 md:py-10">
        <div class="card md:w-lg w-screen z-10">
            <div class="text-center px-10 py-0">
                <!-- Logo -->
                <a class="flex justify-center" href="#">
                    <img alt="logo dark" class="h-6 flex dark:hidden" src="/images/logo-dark.png"/>
                    <img alt="" class="h-6 hidden dark:flex" src="/images/logo-light.png"/>
                </a>
                <div class="mt-3 text-center">
                    <h4 class="mb-2.5 text-xl font-semibold text-primary">Welcome Back !</h4>
                    <p class="text-base text-default-500">Sign in to continue to Tailwick.</p>
                </div>
                <form action="{{route('vendor_login')}}" method="post" class="text-left w-full mt-5">
                    @csrf
                    <div class="mb-4">
                        <label class="block font-medium text-default-900 text-sm mb-2" for="email">Username/ Email
                            ID</label>
                        <input  id="email" placeholder="Enter Username or email" type="text" name="email" class="form-input w-full border-black border-[2px]"/>
                    </div>
                    <div class="mb-4">
                      
                        <label class="block font-medium text-default-900 text-sm mb-2" for="Password">Password</label>
                        <input  id="Password" placeholder="Enter Password" type="text" name="password" class="form-input w-full border-black border-[2px]"/>
                    </div>
                    <div class="flex items-center gap-2 mb-4">
                        <input class="form-checkbox" id="checkbox-1" type="checkbox"/>
                        <label class="text-default-900 text-sm font-medium" for="checkbox-1">Remember Me</label>
                    </div>
                    <div class="mt-4 text-center">
                        <button class="btn bg-blue-600 text-white w-full rounded-[5px] cursor-pointer" type="submit">Sign In</button>
                    </div>
                      <a class="text-primary font-medium text-sm mb-2 float-end"
                           href="#">Forgot Password ?</a>
                    <div
                        class="my-9 relative text-center before:absolute before:top-2.5 before:left-0 before:border-t before:border-t-default-200 before:w-full before:h-0.5 before:right-0 before:-z-0">
                       
                        <h4 class="relative z-1 py-0.5 px-2 inline-block font-medium text-default-600 bg-card mt-4">Sign In
                            With</h4>
                    </div>
                    <div class="flex w-full justify-center items-center gap-2">
                        <a class="btn border border-default-200 flex-grow hover:bg-default-150 shadow-sm hover:text-default-800"
                           href="#">
                            <i class="iconify-color logos--google-icon"></i>
                            Use Google
                        </a>
                        <a class="btn border border-default-200 flex-grow hover:bg-default-150 shadow-sm hover:text-default-800"
                           href="#">
                            <i class="iconify logos--apple text-mono"></i>
                            Use Apple
                        </a>
                    </div>
                    <div class="mt-10 text-center">
                        <p class="text-base text-default-500">Don't have an Account ?
                            <a class="font-semibold underline hover:text-primary transition duration-200"
                               href="#">SignUp</a>
                        </p>
                    </div>
                </form>
            </div>
        </div>
        <div class="absolute inset-0 overflow-hidden">
            <svg aria-hidden="true"
                 class="absolute inset-0 size-full fill-black/2 stroke-black/5 dark:fill-white/2.5 dark:stroke-white/2.5">
                <defs>
                    <pattern height="56" id="authPattern" patternunits="userSpaceOnUse" width="56" x="50%" y="16">
                        <path d="M.5 56V.5H72" fill="none"></path>
                    </pattern>
                </defs>
                <rect fill="url(#authPattern)" height="100%" stroke-width="0" width="100%"></rect>
            </svg>
        </div>
    </div>
@endsection

@section('scripts')

<script src="{{ asset('vendor/js/tailwind.js') }}"></script>
@endsection
